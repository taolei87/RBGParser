...
//	    			(wvMiss + 1e-8)/(wvMiss + wvHit + 1e-8));
    ...
    public boolean isHighOrderFid(int id) {
    	long code = arcAlphabet.lookupCode(id);
    	return extractArcTemplateCode(code) > Arc.FEATURE_1ORDER_END.ordinal();
    }
    ...
    public void dumpParamNorms(Parameters params) {
    	...
    	double max = Double.NEGATIVE_INFINITY, max1st = Double.NEGATIVE_INFINITY, 
    			max2nd = Double.NEGATIVE_INFINITY, max3rd = Double.NEGATIVE_INFINITY,
    			maxhigh = Double.NEGATIVE_INFINITY;
    	double min = Double.POSITIVE_INFINITY, min1st = Double.POSITIVE_INFINITY, 
    			min2nd = Double.POSITIVE_INFINITY, min3rd = Double.POSITIVE_INFINITY, 
    	...
    	for (long code : codes) {
    		...
    		if (temp < Arc.FEATURE_1ORDER_END.ordinal()) {
    			norm1st += value2;
    			max1st = Math.max(max1st, value);
    			min1st = Math.min(min1st, value);
    		} else if (temp < Arc.FEATURE_2ORDER_END.ordinal()) {
    			norm2nd += value2;
    			max2nd = Math.max(max2nd, value);
    			min2nd = Math.min(min2nd, value);
    		} else if (temp < Arc.FEATURE_3ORDER_END.ordinal()) {
    			norm3rd += value2;
    			max3rd = Math.max(max3rd, value);
    			min3rd = Math.min(min3rd, value);
    		} else {
    			normhigh += value2;
